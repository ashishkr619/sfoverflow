...
def question(request):
    ...
    if 'query' in request.GET:
        ...
        if form.is_valid():
            ...
            if query in previous_searches:
                ...
                r = requests.get('https://api.stackexchange.com/2.2/search/advanced?order=desc&sort=activity&q='+query+'&site=stackoverflow')
                answers = r.json()
                for i in range(len(answers['items'])):
                    ...
                    search_term=query
                    question = Question.objects.create(is_answered=is_answered,view_count=view_count,link=link,answer_count=answer_count,score=score,last_activity_date=last_activity_date,creation_date=creation_date,question_id=question_id,title=title,display_name=display_name,search_term=search_term)
    ...
    elif 'query' in request.GET and 'user' in request.GET :
        form = QuestionForm(request.GET)
        if form.is_valid():
            query = form.cleaned_data['query']
            ...
            if query in previous_searches:
                ...
                r = requests.get('https://api.stackexchange.com/2.2/search/advanced?order=desc&sort=activity&q='+query+'&site=stackoverflow'+'&user='+user)
                answers = r.json()
                for i in range(len(answers['items'])):
        ...
        if form.is_valid():
            ...
            if query in previous_searches:
                ...
                r = requests.get('https://api.stackexchange.com/2.2/search/advanced?order=desc&sort=activity&user='+query+'&site=stackoverflow')
                answers = r.json()
                for i in range(len(answers['items'])):
